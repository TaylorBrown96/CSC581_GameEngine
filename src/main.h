...
public:
    ...
    void Update(float deltaTime, InputManager* input) override {
        ...
        velocity.x = 0.0;
        if (input->IsKeyPressed(SDL_SCANCODE_A)||input->IsKeyPressed(SDL_SCANCODE_LEFT)) velocity.x = - 200.0f; // move left
        if (input->IsKeyPressed(SDL_SCANCODE_D)||input->IsKeyPressed(SDL_SCANCODE_RIGHT)) velocity.x = + 200.0f; // move right
...
public:
    ...
    void Update(float dt, InputManager* input) override {
        ...
        if (position.x <= 0 || position.x >= 1920 - dimensions.x) velocity.x *= -1;
    }
